...
export default function OnboardingPage() {
    ...
    const handleOptionSelect = (optionId: string) => {
        ...
        // Store in local storage optionally
        const updatedSelections = { ...selections, [currentStep]: optionId }
        localStorage.setItem("onboardingAnswers", JSON.stringify(updatedSelections))
        ...
        return (
            <div className="min-h-screen w-full flex flex-col items-center justify-center bg-primary p-4 text-primary-foreground">
                ...
                >
                    ...
                    <h2 className="text-3xl font-bold tracking-tight">You’re exactly where you need to be.</h2>
        ...
        <div className="min-h-screen w-full flex flex-col bg-primary items-center p-4 sm:p-6 lg:p-8 relative">
            ...
            <div className="w-full max-w-2xl pt-6 pb-4 sm:pt-12 sm:pb-8 flex flex-col gap-3 shrink-0">
                ...
                <Progress
                    ...
                    className="h-2 rounded-full bg-primary-foreground/20"
                            ...
                            {currentQuestion.options.map((option) => (
                                ...
                                >
                                    <CardContent className="p-5 sm:p-6 flex items-center gap-4">
                                        <div className="flex-shrink-0 w-8 h-8 rounded-full border border-primary/20 flex items-center justify-center text-sm font-bold text-primary bg-primary/10 group-hover:bg-primary group-hover:text-primary-foreground group-hover:border-primary transition-colors">
                                            {option.id}
                                        </div>
                                        <span className="sm:text-lg font-medium leading-snug">{option.text}</span>
                                    </CardContent>
                                </Card>
                            ))}
                        </div>
